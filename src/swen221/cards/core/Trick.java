...
public class Trick implements Serializable {
	...
	public void play(Player p, Card c) throws IllegalMove {
		// Throw error if player doesn't have card
		if(!p.getHand().contains(c))
			throw new IllegalMove("Player does not have card: " + c.toString());

		// Throw error if it's not players turn
		if(p.direction != getNextToPlay())
			throw new IllegalMove("Player played out of turn.");

		// Throw error if player doesnt follow suit but has a valid card
		if(!p.direction.equals(getLeadPlayer()) &&  c.suit() != getCardPlayed(lead).suit()) {
			boolean hasSuit = false;
			for (Card pCard : p.getHand()) {
				if (pCard.suit() == getCardPlayed(lead).suit()) {
					hasSuit = true;
					break;
				}
			}
			...
			if(hasSuit)
				throw new IllegalMove("Player has valid suit but did not play same suit as leader.");
		}
